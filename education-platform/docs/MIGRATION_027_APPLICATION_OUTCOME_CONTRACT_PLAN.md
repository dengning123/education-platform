# Migration 027 Application and Outcome Data Contract Plan
...
`202608250027_application_outcome_contract.sql`

Reserved test identity:
`supabase/tests/019_phase027_application_outcome_contract.sql`

Migration 027 is a provisional future planning number until separate
Application/Outcome implementation authorization. It is not a permanently
reserved slot and no Migration 027 SQL exists.
...
Migration 027 will define a privacy-aware, version-pinned record of a real
...
Migration 027 implementation. Application/Outcome remains planning-only.
...
Migration 027 creates new types rather than altering frozen enums:
...
- discriminator exactly `APPLICATION_SNAPSHOT_V027`;
...
storage/privacy design. Migration 027 retains only typed metadata and a hash.
...
delivery or direct-source confirmation path. Without that path, v027 permits
...
the same lock discipline. Consent is application-specific in v027. It does not
...
```text
create_student_application_v027(...typed arguments...) -> application_id
update_student_application_draft_v027(...typed arguments...) -> void
cancel_student_application_draft_v027(application_id, request_id) -> void
submit_student_application_v027(...pinned IDs..., request_id) -> snapshot result
report_application_outcome_v027(...typed arguments...) -> observation_id
list_application_outcome_review_queue_v027(limit, cursor) -> bounded rows
get_application_outcome_review_candidate_v027(observation_id) -> bounded row
review_application_outcome_v027(...typed arguments...) -> review/selection result
record_application_research_consent_v027(...typed arguments...) -> consent_event_id
```
...
Migration 027 must not modify hosted Supabase platform default ACLs. It revokes
...
All public and private v027 tables are student-owned and cascade from
`students` or `student_applications`. Migration 027 must replace only the
...
exhaustive v027 anti-join inventory.
...
Because v027 stores no raw evidence object or storage locator, it does not
...
under the Phase 4 observability plan. Migration 027 creates no generic log,
...
- no terminal-state reopening in v027;
...
- v027 is additive except for the bounded replacement of the private privacy
...
- authenticated EXECUTE equals the prior whitelist plus exactly the approved
  v027 owner/reviewer functions;

### Privacy and concurrency

- deletion removes every v027 public/private row and retains only the existing
...
- clean PostgreSQL 15 and target PostgreSQL 17 `001→027`;
- populated `026→027` with completed Eligibility/Fit history unchanged;
- ordered SQL suites `001`–`019`;
...
- bulk historical imports or applications lacking the complete v027 submission
  snapshot;
- changes to Phase 3 result semantics;
- implementation or deployment of Migration 027.
...
Until that review and explicit implementation authorization, Migration 027
