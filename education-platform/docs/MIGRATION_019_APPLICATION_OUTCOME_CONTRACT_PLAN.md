# Migration 019 Application and Outcome Data Contract Plan
...
`202608220019_application_outcome_contract.sql`

Reserved test identity:
`supabase/tests/011_phase019_application_outcome_contract.sql`
...
Migration 019 will define a privacy-aware, version-pinned record of a real
...
Migration 019 creates new types rather than altering frozen enums:
...
- discriminator exactly `APPLICATION_SNAPSHOT_V019`;
...
storage/privacy design. Migration 019 retains only typed metadata and a hash.
...
delivery or direct-source confirmation path. Without that path, v019 permits
...
the same lock discipline. Consent is application-specific in v019. It does not
...
review_application_outcome_v019(...typed arguments...) -> review/selection result
record_application_research_consent_v019(...typed arguments...) -> consent_event_id
```
...
- add an exact authenticated-function whitelist assertion to test 011.
...
All public and private v019 tables are student-owned and cascade from
`students` or `student_applications`. Migration 019 must replace only the
current `private.close_student_owned_rows(uuid)` definition, preserving its
identity, owner, callers, search path, and all 001–018 checks while appending an
exhaustive v019 anti-join inventory.
...
Because v019 stores no raw evidence object or storage locator, it does not
...
under the Phase 4 observability plan. Migration 019 creates no generic log,
...
- no terminal-state reopening in v019;
...
- v019 is additive except for the bounded replacement of the private privacy
...
- deletion removes every v019 public/private row and retains only the existing
...
- clean PostgreSQL 15 and target PostgreSQL 17 `001→019`;
- populated `018→019` with completed Eligibility/Fit history unchanged;
...
- bulk historical imports or applications lacking the complete v019 submission
  snapshot;
- changes to Phase 3 result semantics;
- implementation or deployment of Migration 019.
...
Until that review and explicit implementation authorization, Migration 019
