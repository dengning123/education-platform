# Migration 026 Application and Outcome Data Contract Plan
...
`202608250026_application_outcome_contract.sql`

Reserved test identity:
`supabase/tests/018_phase026_application_outcome_contract.sql`

Migration 026 is a provisional future planning number until separate
Application/Outcome implementation authorization. It is not a permanently
reserved slot and no Migration 026 SQL exists.
...
Migration 026 will define a privacy-aware, version-pinned record of a real
...
Migration 026 implementation. Application/Outcome remains planning-only.
...
Migration 026 creates new types rather than altering frozen enums:
...
- discriminator exactly `APPLICATION_SNAPSHOT_V026`;
...
storage/privacy design. Migration 026 retains only typed metadata and a hash.
...
delivery or direct-source confirmation path. Without that path, v026 permits
...
the same lock discipline. Consent is application-specific in v026. It does not
...
```text
create_student_application_v026(...typed arguments...) -> application_id
update_student_application_draft_v026(...typed arguments...) -> void
cancel_student_application_draft_v026(application_id, request_id) -> void
submit_student_application_v026(...pinned IDs..., request_id) -> snapshot result
report_application_outcome_v026(...typed arguments...) -> observation_id
list_application_outcome_review_queue_v026(limit, cursor) -> bounded rows
get_application_outcome_review_candidate_v026(observation_id) -> bounded row
review_application_outcome_v026(...typed arguments...) -> review/selection result
record_application_research_consent_v026(...typed arguments...) -> consent_event_id
```
...
Migration 026 must not modify hosted Supabase platform default ACLs. It revokes
...
All public and private v026 tables are student-owned and cascade from
`students` or `student_applications`. Migration 026 must replace only the
...
exhaustive v026 anti-join inventory.
...
Because v026 stores no raw evidence object or storage locator, it does not
...
under the Phase 4 observability plan. Migration 026 creates no generic log,
...
- no terminal-state reopening in v026;
...
- v026 is additive except for the bounded replacement of the private privacy
...
- authenticated EXECUTE equals the prior whitelist plus exactly the approved
  v026 owner/reviewer functions;

### Privacy and concurrency

- deletion removes every v026 public/private row and retains only the existing
...
- clean PostgreSQL 15 and target PostgreSQL 17 `001→026`;
- populated `025→026` with completed Eligibility/Fit history unchanged;
- ordered SQL suites `001`–`018`;
...
- bulk historical imports or applications lacking the complete v026 submission
  snapshot;
- changes to Phase 3 result semantics;
- implementation or deployment of Migration 026.
...
Until that review and explicit implementation authorization, Migration 026
